...
export function matVecMul (m, v) {
    var nrOfTargetRows = m.length;
    var nrOfTerms = v.length;
    var result = []
    for (let targetRowIndex = 0; targetRowIndex < nrOfTargetRows; targetRowIndex++) {
        result.push (0)
        for (let sourceColumnIndex = 0; sourceColumnIndex < nrOfTerms; sourceColumnIndex++) {
            result [targetRowIndex] += m [targetRowIndex][sourceColumnIndex]
        }
    }
    return result;
}
